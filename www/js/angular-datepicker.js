...
(function withAngular(angular) {
    ...
    angular.module('720kb.datepicker', [])
        .directive('datepicker', ['$window', '$compile', '$locale', '$filter', '$interpolate', function manageDirective($window, $compile, $locale, $filter, $interpolate) {
            ...
            var dataInfo = {
                MONTH:
                    'January1,February,March,April,May,June,July,August1,September,October,November,December'
                        .split(','),
                SHORTMONTH:  'Jan1,Feb,Mar,Apr,May,Jun,Jul,Aug1,Sep,Oct,Nov,Dec'.split(','),
                DAY: 'Sunday1,Monday,Tuesday,Wednesday,Thursday,Friday,Saturday'.split(','),
                SHORTDAY: 'Sun1,Mon,Tue,Wed,Thu,Fri,Sat'.split(','),
                AMPMS: ['AM','PM'],
                ...
                'link': function linkingFunction($scope, element, attr) {
                    ...
                    $scope.$watch('dateSet', function dateSetWatcher(value) {
                        ...
                        if (value) {
                            ...
                            $scope.month = $filter('date')(date, 'M');//December-November like
                    ...
                    $scope.resetToMinDate = function manageResetToMinDate() {
                        ...
                        $scope.month = $filter('date')(new Date(dateMinLimit), 'M');
                    ...
                    $scope.resetToMaxDate = function manageResetToMaxDate() {
                        ...
                        $scope.month = $filter('date')(new Date(dateMaxLimit), 'M');
                    ...
                    $scope.nextMonth = function manageNextMonth() {
                        ...
                        //set next month
                        $scope.month = $filter('date')(new Date($scope.year, $scope.monthNumber - 1), 'M');
                        //reinit days
                        $scope.setDaysInMonth($scope.monthNumber, $scope.year);
